...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function scan(forSelectedPhotos = false) {
    ...
    QRReader.scan(result => {
      ...
      console.log(result);
      if (result.includes('https://www.guucube.com/') && result.indexOf('rewardCode')) {
        let code = result.slice(result.indexOf('rewardCode') + 11)
        location.href = `https://www.guucube.com/pages/reward/store-detail?rewardCode=${code}`;
      } else {
        location.href = `https://www.guucube.com/pages/reward/store-detail?rewardCode=123${result}`;
      }
      // textBoxEle.value = '二維碼有誤，請重新輸入';
      // textBoxEle.select();
